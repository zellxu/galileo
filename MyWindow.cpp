...
void MyWindow::displayTimer(int _val) {
	...
	//Pausing the animation if any particle reachs the ground
	for (int i = 0; i < mWorld->getNumParticles(); i++)
		if( mWorld->getParticle(i)->mPosition.y() <= 0)
			mPlaying = false;
}
...
void MyWindow::draw() {
	...
	//Draw a plane to be the ground
	glBegin(GL_QUADS);
	glNormal3d(0, 1, 1);

	//"underground" to make it easier to see
	glColor4d(0.2, 0.9, 0.2, 1.0);
	glVertex3d(-20, 0, wall);
	glVertex3d(-20, -1, wall);
	glVertex3d(20, 0, wall);
	glVertex3d(20, -1, wall);
	glEnd();
	...
	mTrans[1] = average_height * -1000.f;
